...
test("registro de movimientos manuales mediante API", { skip: !available }, async (t) => {
  ...
  try {
    ...
    let foreignBusiness; let foreignCategory; let foreignProduct; let foreignLocation;
    try {
    foreignBusiness = (await client.query("INSERT INTO businesses(name,slug,created_by,status) VALUES($1,$2,$3,'active') RETURNING id", ["Negocio ajeno movimientos", "negocio-ajeno-movimientos", foreignUser.id])).rows[0];
    ...
    foreignProduct = (await client.query(`INSERT INTO items(sku,name,description,brand,price,stock,category_id,business_id,status) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'active') RETURNING id`, ["MOV-FOR", "Producto ajeno", "Descripción", "Marca", 1, 0, foreignCategory.id, foreignBusiness.id])).rows[0];
    foreignLocation = (await client.query("INSERT INTO business_locations(business_id,name,code,location_type,status,is_default) VALUES($1,$2,$3,'warehouse','active',false) RETURNING id", [foreignBusiness.id, "Ubicación ajena", "AJENA"])).rows[0];
