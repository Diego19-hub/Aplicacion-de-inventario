...
const apiProtectedRegistrationFields = [
  "id",
  "platformRole",
  "role",
  "activeBusinessId",
...
export const apiRegisterValidation = [
  body("username")
    .trim()
    .notEmpty()
    .withMessage("El nombre de usuario es obligatorio.")
    ...
    .withMessage("El usuario debe tener entre 3 y 30 caracteres.")
    .matches(/^[a-zA-Z0-9_]+$/)
    .withMessage("El usuario solamente puede contener letras, números y guion bajo."),
  body("email")
    .trim()
    .notEmpty()
    .withMessage("El correo es obligatorio.")
  ...
  body("password")
    ...
    .withMessage("La contraseña debe tener entre 8 y 64 caracteres.")
    .custom((password) => {
      if (Buffer.byteLength(password, "utf8") > 72) throw new Error("La contraseña es demasiado larga.");
      return true;
    }),
  body("passwordConfirmation")
    ...
    .custom((passwordConfirmation, { req }) => {
      if (passwordConfirmation !== req.body.password) throw new Error("Las contraseñas no coinciden.");
      return true;
    }),
  ...apiProtectedRegistrationFields.map((field) => body(field)
    .not()
    .exists()
