...
export async function createInventoryTransfer({ businessId,itemId,userId,fromLocationId,toLocationId,quantity,reason,reference }) {
 ...
 try { await client.query('BEGIN');
  const item=(await client.query("SELECT id,stock FROM items WHERE id=$1 AND business_id=$2 AND status='active' FOR UPDATE",[itemId,businessId])).rows[0]; if(!item){await client.query('ROLLBACK');return {error:'not_found'};}
  if(fromLocationId===toLocationId){await client.query('ROLLBACK');return {error:'same_location'};}
  const locations=(await client.query("SELECT id FROM business_locations WHERE business_id=$1 AND status='active' AND id=ANY($2::int[]) ORDER BY id FOR KEY SHARE",[businessId,[fromLocationId,toLocationId]])).rows; if(locations.length!==2){await client.query('ROLLBACK');return {error:'location_not_found'};}
  ...
  const transfer=(await client.query("INSERT INTO inventory_transfers(business_id,item_id,from_location_id,to_location_id,quantity,reason,reference,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id",[businessId,itemId,fromLocationId,toLocationId,quantity,reason,reference||null,userId])).rows[0];
  ...
  await client.query('COMMIT'); return transfer;
