...
function oauthErrorRedirect(req, code) {
  ...
  return req.res.redirect(frontendPath(`/login?oauthError=${encodeURIComponent(allowed.has(code) ? code : "GOOGLE_INVALID_TOKEN")}`));
}
...
export async function startGoogleAuth(req, res, next) {
  try {
    ...
    req.session.googleOAuth = { state: request.state, nonce: request.nonce, codeVerifier: request.codeVerifier };
    await saveSession(req);
    return res.redirect(request.url.href);
  } catch (error) {
    if (error.code === "GOOGLE_NOT_CONFIGURED") return oauthErrorRedirect(req, error.code);
    return next(error);
  }
}
...
export async function googleCallback(req, res, next) {
  ...
  if (req.query.error === "access_denied") return oauthErrorRedirect(req, "GOOGLE_CANCELLED");
  if (!oauthSession || typeof req.query.code !== "string" || typeof req.query.state !== "string" || req.query.state !== oauthSession.state) return oauthErrorRedirect(req, "GOOGLE_INVALID_STATE");
  try {
    ...
    if (!user) {
      ...
      if (user) {
        ...
        if (linked && linked.id !== user.id) return oauthErrorRedirect(req, "GOOGLE_ACCOUNT_CONFLICT");
    ...
    await establishAuthenticatedSession(req, user, { activeBusinessId });
    return res.redirect(frontendPath(businesses.length === 1 ? "/app" : "/select-business"));
  } catch (error) {
    logGoogleCallbackError(error);
    if (["GOOGLE_INVALID_STATE", "GOOGLE_EMAIL_NOT_VERIFIED", "GOOGLE_INVALID_ISSUER", "GOOGLE_INVALID_AUDIENCE", "AuthorizationResponseError", "ResponseBodyError"].includes(error.code) || error.name === "AuthorizationResponseError") return oauthErrorRedirect(req, error.code === "GOOGLE_EMAIL_NOT_VERIFIED" ? error.code : "GOOGLE_INVALID_TOKEN");
    if (error.code === "23505") return oauthErrorRedirect(req, "GOOGLE_ACCOUNT_CONFLICT");
    if (error.code === "42703" || error.code === "42P01") return oauthErrorRedirect(req, "GOOGLE_SCHEMA_NOT_READY");
    return next(error);
  }
}
