...
export async function createProductMovement(req, res, next) {
  ...
  try {
    ...
    if (movement.error === "fifo_adjustment_cost_required") return res.status(422).json({ error: { code: "FIFO_ADJUSTMENT_COST_REQUIRED", message: "En FIFO, un ajuste positivo requiere costo unitario. Si no se conoce, registra costo 0 con el motivo autorizado de existencia sin costo conocido.", fields: [{ field: "unitCost", message: "Indica el costo unitario del ajuste positivo." }] } });
    if (movement.error === "fifo_zero_cost_reason_required") return res.status(422).json({ error: { code: "FIFO_ZERO_COST_REASON_REQUIRED", message: "El costo cero solo está permitido con el motivo autorizado de existencia sin costo conocido; afectará la valuación del inventario y puede afectar la utilidad.", fields: [{ field: "zeroCostReason", message: "Selecciona el motivo autorizado para costo cero." }] } });
    if (movement.error === "fifo_zero_cost_reason_invalid") return res.status(422).json({ error: { code: "FIFO_ZERO_COST_REASON_INVALID", message: "El motivo de costo cero solo puede usarse cuando el costo unitario es 0.", fields: [{ field: "zeroCostReason", message: "Quita el motivo o usa costo 0." }] } });
