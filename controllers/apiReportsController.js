...
export async function movementReportApi(req, res, next) {
  ...
  const dateFrom = req.query.startDate || req.query.dateFrom || ""; const dateTo = req.query.endDate || req.query.dateTo || "";
  ...
  try {
    const options = await movementOptions(req.business.id); const locationId = id(req.query.locationId || req.query.location); const userId = id(req.query.userId || req.query.user); const categoryId = id(req.query.categoryId || req.query.category); const productId = id(req.query.productId);
    const movementType = ["opening_balance", "entry", "exit", "adjustment", "transfer_out", "transfer_in"].includes(req.query.movementType) ? req.query.movementType : "";
    const filters = { businessId: req.business.id, role: req.membership.role, q: normalizeSearch(req.query.search ?? req.query.q ?? "").slice(0, 100), productId: !productId ? null : options.products.some((x) => Number(x.id) === productId) ? productId : -1, locationId: !locationId ? null : options.locations.some((x) => Number(x.id) === locationId) ? locationId : -1, userId: !userId ? null : options.users.some((x) => Number(x.id) === userId) ? userId : -1, categoryId: !categoryId ? null : options.categories.some((x) => Number(x.id) === categoryId) ? categoryId : -1, type: movementType, dateFrom, dateTo, limit: 25, offset: 0 };
    ...
    return res.json({ data: { movements: result.rows.map((row) => ({ id: Number(row.id), createdAt: row.created_at, type: row.movement_type, movementType: row.movement_type, quantityDelta: Number(row.quantity_delta), previousStock: Number(row.previous_stock), resultingStock: Number(row.resulting_stock), reason: row.reason, reference: row.reference, categoryId: Number(row.category_id), categoryName: row.category_name, barcode: row.barcode, product: { id: Number(row.item_id), name: row.item_name, sku: row.sku, status: row.product_status }, location: { id: Number(row.location_id), name: row.location_name, code: row.code }, createdBy: { id: Number(row.created_by), username: row.username }, transferId: row.transfer_id === null ? null : Number(row.transfer_id) })), products: options.products.map((x) => ({ id: Number(x.id), name: x.name, sku: x.sku })), categories: options.categories.map((x) => ({ id: Number(x.id), name: x.name })), locations: options.locations.map((x) => ({ id: Number(x.id), name: x.name, code: x.code })), users: options.users.map((x) => ({ id: Number(x.id), username: x.username })), filters: { search: filters.q, startDate: dateFrom, endDate: dateTo, productId: filters.productId, locationId: filters.locationId, userId: filters.userId, categoryId: filters.categoryId, movementType: filters.type }, pagination: { page, pageSize: 25, totalItems: Number(result.count), totalPages }, permissions: { canViewArchived: req.membership.role === "owner" } } });
  } catch (error) {
    if (process.env.NODE_ENV === "development") console.error("[reports/movements]", { code: error.code, message: error.message, detail: error.detail, stack: error.stack });
    return next(error);
  }
}
...
export async function movementCsvApi(req, res, next) {
  ...
  try {
    const options = await movementOptions(req.business.id); const locationId = id(req.query.locationId || req.query.location); const userId = id(req.query.userId || req.query.user); const categoryId = id(req.query.categoryId || req.query.category); const productId = id(req.query.productId); const type = ["opening_balance","entry","exit","adjustment","transfer_out","transfer_in"].includes(req.query.movementType) ? req.query.movementType : "";
    const rows = await movementExport({ businessId:req.business.id, role:req.membership.role, q:normalizeSearch(req.query.search ?? req.query.q ?? "").slice(0,100), productId:!productId ? null : options.products.some((x)=>Number(x.id)===productId) ? productId : -1, categoryId:!categoryId ? null : options.categories.some((x)=>Number(x.id)===categoryId) ? categoryId : -1, locationId:!locationId ? null : options.locations.some((x)=>Number(x.id)===locationId) ? locationId : -1, userId:!userId ? null : options.users.some((x)=>Number(x.id)===userId) ? userId : -1, type, dateFrom, dateTo }); const labels={opening_balance:"Saldo inicial",entry:"Entrada",exit:"Salida",adjustment:"Ajuste",transfer_out:"Transferencia — salida",transfer_in:"Transferencia — entrada"};
