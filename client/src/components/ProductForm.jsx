...
export function ProductForm({
  ...
  return <form className="product-form" onSubmit={onSubmit} noValidate>
    ...
    <div className="product-form__fields">
      ...
      <Select id="product-category" label="Categoría *" value={form.categoryId} onChange={(event) => onChange("categoryId", event.target.value)} required error={errors.categoryId}><option value="">Selecciona una categoría</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</Select>
