...
export function ArchivedProductDetailsPage() {
  ...
  return <>
    ...
    <Card className="detail-movements"><header className="section-heading"><div><p className="eyebrow">Actividad conservada</p><h2>Movimientos recientes</h2></div></header>{recentMovements.length === 0 ? <EmptyState title="Sin movimientos recientes" description="No hay movimientos para este producto." /> : <div className="movement-list">{recentMovements.map((movement) => <article className="movement-row" key={movement.id}><div><strong>{movementLabels[movement.type] ?? movement.type}</strong><span>{movement.location.name} ({movement.location.code})</span></div><div><strong>{movement.quantityDelta >= 0 ? "+" : ""}{movement.quantityDelta}</strong><span>{movement.previousStock} → {movement.resultingStock}</span></div><div><span>{movement.createdBy.username}</span><time dateTime={movement.createdAt}>{formatDate(movement.createdAt)}</time>{movement.transferId !== null && <Link className="text-link" to={`/app/transfers/${movement.transferId}`}>Transferencia #{movement.transferId}</Link>}</div><p className="movement-reason">{movement.reason}</p></article>)}</div>}</Card>
  </>;
}
