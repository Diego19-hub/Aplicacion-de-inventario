...
export function ArchivedProductsPage() {
  ...
  return <>
    ...
    {!loading && data?.products.length > 0 && <><section className={`product-grid ${viewMode === "list" ? "resource-list" : ""}`} aria-label="Productos archivados">{data.products.map((product) => <Card key={product.id} className="product-card"><div className="product-card__heading"><span className="sku-badge">{product.sku}</span><span>{product.category.name}</span></div><h2>{product.name}</h2><p className="muted">{product.brand || "Sin marca"}</p><dl><div><dt>Existencias</dt><dd>{product.stock} unidades</dd></div><div><dt>Archivado</dt><dd><time dateTime={product.archivedAt}>{formatDate(product.archivedAt)}</time></dd></div></dl><p className="muted">Motivo: {product.reason}</p><Link className="text-link" to={`/app/products/${product.id}/archived`}>Ver detalle</Link></Card>)}</section>{data.pagination.totalPages > 1 && <nav className="product-pagination" aria-label="Paginación de archivados">{pages(data.pagination.page, data.pagination.totalPages).map((page) => <Button key={page} variant={page === data.pagination.page ? "primary" : "secondary"} aria-current={page === data.pagination.page ? "page" : undefined} onClick={() => update({ q: data.filters.q, category: data.filters.categoryId ?? "", page })}>{page}</Button>)}</nav>}</>}
  </>;
}
