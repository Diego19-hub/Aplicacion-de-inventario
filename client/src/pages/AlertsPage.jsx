...
export function AlertsPage() {
  ...
  return <>
    ...
    {!loading && data && <>
      ...
      {data.alerts.length === 0 ? <EmptyState title={data.pagination.totalItems === 0 && !data.filters.q && !data.filters.categoryId && !data.filters.locationId ? "Sin alertas activas" : "Sin coincidencias"} description={data.pagination.totalItems === 0 ? "Puede que no existan umbrales configurados o que las existencias estén por encima del mínimo." : "Prueba con filtros diferentes."} /> : <section className="category-api-grid" aria-label="Alertas de existencias">{data.alerts.map((alert) => <Card key={alert.thresholdId} className="category-api-card"><div><p className="eyebrow">{labels[alert.status]}</p><h2><Link className="text-link" to={`/app/products/${alert.product.id}`}>{alert.product.name}</Link></h2><p className="muted">{alert.product.sku} · {alert.product.category.name}</p>{data.permissions.canManageThresholds && <Link className="text-link" to={`/app/products/${alert.product.id}/thresholds`}>Configurar alertas</Link>}</div><dl><div><dt>Ubicación</dt><dd>{alert.location.name} ({alert.location.code})</dd></div><div><dt>Stock local</dt><dd>{alert.stock}</dd></div><div><dt>Mínimo</dt><dd>{alert.minimumStock}</dd></div><div><dt>Estado</dt><dd><AlertTriangle aria-hidden="true" /> {labels[alert.status]}</dd></div></dl></Card>)}</section>}
