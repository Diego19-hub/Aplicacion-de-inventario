...
export function SuppliersPage() {
  ...
  const [viewMode, setViewMode] = useState(() => getStoredViewMode("suppliers_view_mode"));
  ...
  return <>
    ...
    <ViewModeToggle value={viewMode} storageKey="suppliers_view_mode" onChange={setViewMode} />
    {suppliers.length === 0 ? <EmptyState title={hasFilters ? "Sin coincidencias" : "Sin proveedores activos"} description={hasFilters ? "Prueba con otros filtros." : "Los proveedores del negocio aparecerán aquí."} action={hasFilters ? <Button variant="secondary" onClick={clear}>Limpiar filtros</Button> : null} /> : <section className={`category-api-grid ${viewMode === "list" ? "resource-list" : ""}`} aria-label="Listado de proveedores">{suppliers.map((supplier) => <Card key={supplier.id} className="category-api-card"><div><h2>{supplier.name}</h2><p className="muted">{supplier.legalName || "Sin razón social registrada"}</p></div><dl><div><dt>Contacto</dt><dd>{supplier.contactName || "Sin contacto"}</dd></div><div><dt>Correo o teléfono</dt><dd>{supplier.email || supplier.phone || "Sin datos de contacto"}</dd></div><div><dt>RFC</dt><dd>{supplier.taxId || "Sin RFC"}</dd></div><div><dt>Estado</dt><dd>{statusLabel(supplier.status)}</dd></div></dl><Link className="text-link" to={`/app/suppliers/${supplier.id}`}>Ver detalle</Link></Card>)}</section>}
