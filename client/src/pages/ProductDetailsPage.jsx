...
export function ProductDetailsPage() {
  ...
  return <>
    ...
    <PageHeader
      ...
      actions={<>{session.permissions.canManageInventory && <><Link className="button button--primary" to={`/app/products/${product.id}/edit`}>Editar producto</Link><Link className="button button--secondary" to={`/app/products/${product.id}/movements/new`}>Registrar movimiento</Link><Link className="button button--secondary" to={`/app/transfers/new?product=${product.id}`}>Transferir</Link></>}{session.permissions.canDeleteInventory && <Link className="button button--danger" to={`/app/products/${product.id}/archive`}>Archivar producto</Link>}</>}
    />
    ...
    <Card className="detail-movements"><header className="section-heading"><div><p className="eyebrow">Actividad</p><h2>Movimientos recientes</h2></div><Link className="text-link" to={`/app/products/${product.id}/movements`}>Ver todos los movimientos</Link></header>{recentMovements.length === 0 ? <EmptyState title="Sin movimientos recientes" description="Los movimientos de este producto aparecerán aquí." /> : <div className="movement-list">{recentMovements.map((movement) => <article className="movement-row" key={movement.id}><div><strong>{movementLabels[movement.type] ?? movement.type}</strong><span>{movement.location.name} ({movement.location.code})</span></div><div><strong className={movement.quantityDelta >= 0 ? "delta delta--positive" : "delta delta--negative"}>{movement.quantityDelta >= 0 ? "+" : ""}{movement.quantityDelta}</strong><span>{movement.previousStock} → {movement.resultingStock}</span></div><div><span>{movement.createdBy.username}</span><time dateTime={movement.createdAt}>{formatDate(movement.createdAt)}</time>{movement.transferId !== null && <small>Transferencia #{movement.transferId}</small>}</div><p className="movement-reason">{movement.reason}</p></article>)}</div>}</Card>
  </>;
}
