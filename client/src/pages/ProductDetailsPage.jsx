...
const alertLabels = { not_configured: "Sin umbral configurado", out_of_stock: "Agotado", low_stock: "Stock bajo", overstock: "Stock excedente", ok: "Stock suficiente" };
...
export function ProductDetailsPage() {
  ...
  return <>
    ...
    <PageHeader
      title={product.name}
      description={`SKU: ${product.sku}${product.barcode ? ` · Código de barras: ${product.barcode}` : ""}`}
    ...
    <section className="product-detail-grid">
      <Card><p className="eyebrow">Información del producto</p><p className="product-description">{product.description || "Este producto no tiene una descripción registrada."}</p><dl className="detail-list"><div><dt>Categoría</dt><dd>{product.category.name}</dd></div><div><dt>Marca</dt><dd>{product.brand}</dd></div><div><dt>Precio</dt><dd>{new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.price)}</dd></div><div><dt>Costo de adquisición</dt><dd>{product.costPrice === null ? "No configurado" : new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.costPrice)}</dd></div>{product.costPrice !== null && <div><dt>Margen estimado</dt><dd className={product.price - product.costPrice >= 0 ? "delta delta--positive" : "delta delta--negative"}>{new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.price - product.costPrice)} por unidad</dd></div>}<div><dt>Existencias totales</dt><dd>{product.stock} unidades</dd></div><div><dt>Creado</dt><dd><time dateTime={product.createdAt}>{formatDate(product.createdAt)}</time></dd></div></dl></Card>
