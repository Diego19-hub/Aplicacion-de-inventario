...
export function CategoriesPage() {
  ...
  return <>
    ...
    {!isLoading && data && data.categories.length > 0 && <><section className="category-api-grid" aria-label="Lista de categorías">{data.categories.map((category) => <Card key={category.id} className="category-api-card"><h2>{category.name}</h2><p className="muted">{category.description || "Sin descripción."}</p><dl><div><dt>Productos activos</dt><dd>{category.activeProductCount}</dd></div><div><dt>Productos archivados</dt><dd>{category.archivedProductCount}</dd></div><div><dt>Existencias activas</dt><dd>{category.totalStock} unidades</dd></div></dl><Link className="text-link" to={`/app/categories/${category.id}`}>Ver detalle</Link></Card>)}</section>{data.pagination.totalPages > 1 && <nav className="product-pagination" aria-label="Paginación de categorías">{pageNumbers(data.pagination.page, data.pagination.totalPages).map((page, index, pages) => <span key={page}>{index > 0 && page - pages[index - 1] > 1 && <span aria-hidden="true">…</span>}<Button variant={page === data.pagination.page ? "primary" : "secondary"} aria-current={page === data.pagination.page ? "page" : undefined} onClick={() => updateFilters({ q: data.filters.q, page })}>{page}</Button></span>)}</nav>}</>}
  </>;
}
