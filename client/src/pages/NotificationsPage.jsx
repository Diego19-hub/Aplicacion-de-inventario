...
export function NotificationsPage() {
  ...
  const [data, setData] = useState(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [markingAll, setMarkingAll] = useState(false); const [markAllMessage, setMarkAllMessage] = useState("");
  const load = useCallback(async (requestParams = params) => { setLoading(true); setError(""); try { const query = requestParams.toString(); setData(await apiRequest(`/notifications${query ? `?${query}` : ""}`)); return true; } catch (e) { if (e.name !== "AbortError") setError(e.message || "No fue posible cargar las notificaciones."); return false; } finally { setLoading(false); } }, [params]);
  useEffect(() => { load(); }, [load]);
  ...
  async function markAll() {
    ...
    setError("");
    try {
      const result = await apiRequest("/notifications/read-all", { method: "PATCH", csrf: true });
      const updated = Number.isFinite(Number(result?.updated)) ? Number(result.updated) : 0;
      window.dispatchEvent(new CustomEvent("notifications:all-read", { detail: { unreadCount: 0 } }));
      const refreshedParams = new URLSearchParams(params);
      if (refreshedParams.has("page")) refreshedParams.set("page", "1");
      if (refreshedParams.toString() !== params.toString()) setParams(refreshedParams, { replace: true, preventScrollReset: true });
      const loaded = await load(refreshedParams);
      if (!loaded) setError("No pudimos marcar las notificaciones como leídas. Intenta nuevamente.");
      else setMarkAllMessage(updated > 0 ? `Se marcaron ${updated} notificaciones como leídas` : "No hay notificaciones pendientes");
    } catch {
      setError("No pudimos marcar las notificaciones como leídas. Intenta nuevamente.");
    } finally {
      setMarkingAll(false);
    }
  }
  ...
  return <div className="notifications-page"><PageHeader title="Notificaciones" description="Consulta los avisos importantes del negocio." actions={<Button variant="secondary" onClick={markAll} disabled={markingAll}>{markingAll ? "Marcando como leídas…" : <><CheckCheck aria-hidden="true" />Marcar todas como leídas</>}</Button>} />
