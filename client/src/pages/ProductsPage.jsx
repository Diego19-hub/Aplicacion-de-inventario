...
export function ProductsPage() {
  ...
  return <>
    ...
    {!isLoading && data && data.products.length > 0 && <>
      <section className="product-grid" aria-label="Lista de productos">{data.products.map((product) => <Card key={product.id} className="product-card"><div className="product-card__heading"><span className="sku-badge">{product.sku}</span><span>{product.category.name}</span></div><h2>{product.name}</h2><p className="muted">{product.brand}</p><dl><div><dt>Precio</dt><dd>{new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.price)}</dd></div><div><dt>Existencias</dt><dd>{product.stock} unidades</dd></div></dl><Link to={`/app/products/${product.id}`} className="text-link">Ver detalle</Link></Card>)}</section>
