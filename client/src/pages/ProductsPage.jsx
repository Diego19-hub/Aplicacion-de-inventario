...
export function ProductsPage() {
  ...
  function handleBarcodeDetected(code) {
    setQuery(code);
    updateFilters({ q: code, category, page: 1 });
  }
  ...
  return <><HelpInfoPanel moduleKey="inventory" />
    <PageHeader
      title="Productos"
    ...
    <Card className="product-filter-card">
      <form className="products-filter" onSubmit={handleSubmit}>
        <div className="products-filter-fields">
          <div className="products-search-field"><Input id="product-search" label="Buscar productos" type="search" placeholder="Nombre, SKU o código de barras" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
          <div className="products-barcode-field"><BarcodeScanner label="Escanear código" onDetected={handleBarcodeDetected} /></div>
          <div className="products-category-field"><Select id="product-category" label="Categoría" value={category} onChange={(event) => setCategory(event.target.value)}><option value="">Todas las categorías</option>{data?.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
        </div>
        <div className="products-filter-actions"><Button type="submit" className="search-button"><Search aria-hidden="true" /><span>Buscar</span></Button><Button type="button" variant="secondary" className="filter-clear-button" onClick={clearFilters}>Limpiar filtros</Button></div>
      </form>
    </Card>
    ...
    {!isLoading && data && data.products.length > 0 && <>
      {viewMode === "list" ? <Card className="product-list-card"><div className="product-table-wrap product-table-wrap--products"><table className="product-table"><caption className="visually-hidden">Productos</caption><thead><tr><th>Producto</th><th>SKU</th><th>Código de barras</th><th>Categoría</th><th className="product-table__numeric">Precio</th><th>Stock</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{data.products.map((product) => <tr key={product.id}><th scope="row"><span className="product-table__name">{product.name}</span>{product.brand && <span className="product-table__brand">{product.brand}</span>}</th><td><span className="product-table__secondary">{product.sku}</span></td><td><span className="product-table__secondary product-barcode">{product.barcode || "Sin configurar"}</span></td><td><span className="product-table__secondary">{product.category.name}</span></td><td className="product-table__numeric">{new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.price)}</td><td>{stockIndicator(product.stock)}</td><td>{activeStatus()}</td><td><div className="product-table__actions"><Link className="button button--primary button--compact" to={`/app/products/${product.id}`}>Ver detalle</Link>{(session.permissions.canManageInventory || session.permissions.canDeleteInventory) && actionMenu(product)}</div></td></tr>)}</tbody></table></div></Card> : <section className="product-grid" aria-label="Productos en tarjetas">{data.products.map((product) => <Card key={product.id} className="product-card"><div className="product-card__heading"><span className="sku-badge">{product.sku}</span><span>{product.category.name}</span></div><h2>{product.name}</h2><p className="muted">{product.brand || "Sin marca"}</p><p className="product-card__barcode">Código de barras: {product.barcode || "Sin configurar"}</p><dl><div><dt>Precio</dt><dd>{new Intl.NumberFormat("es-MX", { style: "currency", currency }).format(product.price)}</dd></div><div><dt>Stock</dt><dd>{stockIndicator(product.stock)}</dd></div><div><dt>Estado</dt><dd>{activeStatus()}</dd></div></dl>{cardActions(product)}</Card>)}</section>}
