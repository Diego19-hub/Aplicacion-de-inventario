...
export function BreakEvenPage() {
  ...
  return <section className="break-even-page">
    ...
    <Card className="break-even-help-card"><div className="break-even-help-heading"><div><p className="eyebrow">Información</p><h2>¿Cómo se calcula el punto de equilibrio?</h2></div><Button variant="secondary" aria-expanded={isExplanationOpen} aria-controls="break-even-explanation" onClick={() => setIsExplanationOpen((open) => !open)}>{isExplanationOpen ? "Ocultar explicación" : "Ver explicación"}</Button></div>{isExplanationOpen && <div id="break-even-explanation" className="break-even-explanation"><p><strong>Costos fijos:</strong> gastos operativos del periodo.</p><p><strong>Costos variables:</strong> costo real de productos vendidos.</p>{valuationMethod === "fifo" ? <p>Los costos variables se calculan con el costo real de las capas FIFO consumidas por las ventas, empezando por las capas más antiguas.</p> : valuationMethod === "average" ? <p>Los costos variables se calculan usando el costo promedio de los productos vendidos.</p> : <p>Los costos variables se calculan según el método de valuación activo.</p>}{valuationMethod === "fifo" && <p><strong>Nota FIFO:</strong> El resultado puede variar cuando cambian los costos de las capas disponibles.</p>}<p><strong>Utilidad bruta:</strong> ventas menos costo variable.</p><p><strong>Margen de contribución:</strong> ventas menos costo variable.</p><div className="break-even-formulas"><p><strong>Punto de equilibrio en unidades</strong> = costos fijos / margen por unidad</p><p><strong>Punto de equilibrio en dinero</strong> = costos fijos / porcentaje de margen</p></div></div>}</Card>
    ...
    <section className="break-even-summary-grid" aria-label="Resumen financiero">
      ...
      <Card><span>Costos variables</span><small className="break-even-help-text">Costo real de productos vendidos.</small><strong>{money(result.variableCosts, currency)}</strong></Card>
