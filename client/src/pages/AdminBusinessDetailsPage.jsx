...
export function AdminBusinessDetailsPage() {
  ...
  return <>
    ...
    <section className="category-products"><header className="section-heading"><div><p className="eyebrow">Actividad reciente</p><h2>Últimos movimientos</h2></div></header>{recentMovements.length === 0 ? <EmptyState title="Sin movimientos recientes" description="Los movimientos recientes aparecerán aquí." /> : <section className="transfer-list" aria-label="Movimientos recientes">{recentMovements.map((movement) => <Card key={movement.id} className="transfer-card"><div className="transfer-card__header"><div><strong>{movement.product.name}</strong><span>{movement.product.sku}</span></div><strong>{movement.quantityDelta > 0 ? "+" : ""}{movement.quantityDelta}</strong></div><div className="transfer-card__meta"><span>{movementLabel(movement.type)}</span><span>{movement.location.name} ({movement.location.code})</span><span>{movement.createdBy.username}</span><time dateTime={movement.createdAt}>{formatDate(movement.createdAt)}</time></div></Card>)}</section>}</section>
  </>;
}
