...
const money = (value, currency = "MXN", missingLabel = "Costo no disponible") => formatSafeMoney(value, currency, missingLabel);
...
export function TransactionsPage() {
  ...
  return <section className="transactions-page"><PageHeader title="Transacciones" description="Consulta en un solo lugar las operaciones de inventario, ventas, transferencias y producción." actions={canManage && <div className="transactions-header-actions"><Link className="button button--primary" to="/app/transactions/entries/new">Nueva entrada</Link><Link className="button button--secondary" to="/app/transactions/adjustments/new">Nuevo ajuste</Link></div>} /><ValuationMethodInfo /><TransactionHelpCard /><Card><form className="transactions-filters" onSubmit={handleSubmit}><Input id="transactions-q" name="q" label="Buscar" placeholder="Producto, SKU o referencia" type="text" value={draftFilters.q ?? ""} onChange={(event) => updateDraftFilter("q", event.target.value)} onKeyDown={handleKeyDown} /><Select id="transactions-type" label="Tipo" value={draftFilters.type ?? ""} onChange={(event) => updateDraftFilter("type", event.target.value)}><option value="">Todos</option>{Object.entries(labels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</Select><Input id="transactions-from" label="Desde" type="date" value={draftFilters.dateFrom ?? ""} onChange={(event) => updateDraftFilter("dateFrom", event.target.value)} /><Input id="transactions-to" label="Hasta" type="date" value={draftFilters.dateTo ?? ""} onChange={(event) => updateDraftFilter("dateTo", event.target.value)} /><Select id="transactions-location" label="Ubicación" value={draftFilters.locationId ?? ""} onChange={(event) => updateDraftFilter("locationId", event.target.value)}><option value="">Todas</option>{options.locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</Select><Select id="transactions-user" label="Usuario" value={draftFilters.userId ?? ""} onChange={(event) => updateDraftFilter("userId", event.target.value)}><option value="">Todos</option>{options.users.map((user) => <option key={user.id} value={user.id}>{user.username}</option>)}</Select><div className="filter-actions"><Button type="submit" className="filter-apply-button"><Search aria-hidden="true" /><span>Aplicar</span></Button><Button type="button" variant="secondary" className="filter-clear-button" onClick={clearFilters}>Limpiar</Button></div></form></Card>{transactions.length === 0 ? <EmptyState title="Sin transacciones" description="No hay operaciones que coincidan con los filtros seleccionados." /> : <><Card className="transactions-table-card"><div className="transactions-table-wrap"><table className="transactions-table"><thead><tr><th>Fecha</th><th>Tipo</th><th>Referencia</th><th>Motivo</th><th>Producto</th><th>Cantidad</th><th>Origen</th><th>Destino</th><th>Usuario</th><th>Costo total</th><th>Estado</th></tr></thead><tbody>{transactions.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} currency={currency} />)}</tbody></table></div></Card><div className="transactions-cards">{transactions.map((transaction) => <TransactionCard key={transaction.id} transaction={transaction} currency={currency} />)}</div><Pagination page={page} totalPages={totalPages} onPageChange={goToPage} /></>}</section>;
}
function TransactionRow({ transaction, currency }) { const Icon = icons[transaction.type] || Filter; return <tr><td><Link to={`/app/transactions/${transaction.id}`}>{new Date(transaction.date).toLocaleString("es-MX")}</Link></td><td><span className="transaction-type"><Icon aria-hidden="true" />{labels[transaction.type]}</span></td><td>{transaction.reference || "—"}</td><td>{transaction.reason || "—"}</td><td><strong>{transaction.product.name}</strong><small>{transaction.product.sku}</small></td><td className={transaction.quantity < 0 ? "transaction-negative" : ""}>{transaction.quantity > 0 ? "+" : ""}{transaction.quantity}</td><td>{transaction.origin?.name || transaction.location?.name || "—"}</td><td>{transaction.destination?.name || "—"}</td><td>{transaction.user.username}</td><td>{money(transaction.cost, currency)}</td><td>{transaction.status === "completed" ? "Completada" : transaction.status}</td></tr>; }
function TransactionCard({ transaction, currency }) { return <Link className="transaction-card" to={`/app/transactions/${transaction.id}`}><strong>{transaction.product.name}</strong><span>{labels[transaction.type]} · {new Date(transaction.date).toLocaleDateString("es-MX")}</span><span>SKU: {transaction.product.sku}</span><span>Referencia: {transaction.reference || "—"}</span><span>Motivo: {transaction.reason || "—"}</span><b>Cantidad: {transaction.quantity}</b><span>{transaction.origin?.name || transaction.location?.name || "—"}{transaction.destination ? ` → ${transaction.destination.name}` : ""}</span><span>Costo total: {money(transaction.cost, currency)} · {transaction.user.username}</span></Link>; }
function TransactionDetail({ transaction }) { const { session } = useAuth(); const currency = session.activeBusiness?.currency ?? "MXN"; return <section className="transactions-page"><PageHeader title="Detalle de transacción" description="Información de la operación registrada." actions={<Link className="button button--secondary" to="/app/transactions">Volver a transacciones</Link>} /><ValuationMethodInfo /><Card className="transaction-detail"><h2>{labels[transaction.type]}</h2><dl><div><dt>Producto</dt><dd>{transaction.product.name} · {transaction.product.sku}</dd></div><div><dt>Fecha</dt><dd>{new Date(transaction.date).toLocaleString("es-MX")}</dd></div><div><dt>Referencia</dt><dd>{transaction.reference || "—"}</dd></div><div><dt>Cantidad</dt><dd>{transaction.quantity}</dd></div><div><dt>Costo unitario</dt><dd>{money(transaction.unitCost, currency)}</dd></div><div><dt>Costo total retirado</dt><dd>{money(transaction.cost, currency)}</dd></div><div><dt>Método</dt><dd>{valuationLabels[transaction.valuationMethod] || "Consultar método activo"}</dd></div><div><dt>Estado</dt><dd>{transaction.status}</dd></div><div><dt>Ubicación origen</dt><dd>{transaction.origin?.name || transaction.location?.name || "—"}</dd></div><div><dt>Ubicación destino</dt><dd>{transaction.destination?.name || "—"}</dd></div><div><dt>Usuario responsable</dt><dd>{transaction.user.username}</dd></div><div><dt>Observaciones</dt><dd>{transaction.reason || "—"}</dd></div></dl></Card>{transaction.type === "exit" && <TransactionCostLayers transaction={transaction} currency={currency} />}</section>; }
function TransactionCostLayers({ transaction, currency }) {
  const [state, setState] = useState({ loading: true, error: null, data: null });
  useEffect(() => {
    let active = true;
    setState({ loading: true, error: null, data: null });
    apiRequest(`/products/${transaction.product.id}/cost-layers`)
      .then((data) => { if (active) setState({ loading: false, error: null, data }); })
      .catch((error) => { if (active) setState({ loading: false, error, data: null }); });
    return () => { active = false; };
  }, [transaction.product.id, transaction.reference]);

  if (state.loading) return <Card className="transaction-cost-layers"><Spinner label="Cargando capas FIFO consumidas" /></Card>;
  if (state.error) return <Card className="transaction-cost-layers"><Alert>No fue posible cargar las capas FIFO consumidas.</Alert></Card>;
  if (state.data?.valuationMethod === "average") return <Card className="transaction-cost-layers"><p className="muted">Este negocio utiliza valuación promedio; no hay capas FIFO para mostrar.</p></Card>;
  const consumptions = (state.data?.consumptions ?? []).filter((consumption) => consumption.reference === transaction.reference);
  if (consumptions.length === 0) return <Card className="transaction-cost-layers"><EmptyState title="Sin capas FIFO consumidas" description="No hay consumos FIFO relacionados con esta salida." /></Card>;
  return <Card className="transaction-cost-layers"><div className="section-heading"><div><p className="eyebrow">Trazabilidad</p><h2>Capas FIFO consumidas</h2></div><span className="muted">{valuationLabels[state.data.valuationMethod] || "PEPS/FIFO"}</span></div><div className="transaction-cost-layers__grid">{consumptions.map((consumption) => <div className="transaction-cost-layer" key={consumption.id}><strong>Capa #{consumption.layerId}</strong><span>Fecha: {new Date(consumption.consumedAt).toLocaleString("es-MX")}</span><span>Tipo de operación: {labels[transaction.type] || transaction.type}</span><span>Cantidad: {consumption.quantity}</span><span>Costo unitario: {money(consumption.unitCost, currency)}</span><span>Costo total: {money(consumption.totalCost, currency)}</span><span>Referencia: {consumption.reference || "—"}</span></div>)}</div></Card>;
}
