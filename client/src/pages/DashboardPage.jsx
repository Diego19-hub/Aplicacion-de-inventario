...
async function fetchSales(range, signal) { const first = await apiRequest(`/sales?status=completed&dateFrom=${range.from}&dateTo=${range.to}&limit=50&page=1`, { signal }); const sales = [...(first.sales || [])]; for (let page = 2; page <= (first.pagination?.totalPages || 1); page += 1) { const next = await apiRequest(`/sales?status=completed&dateFrom=${range.from}&dateTo=${range.to}&limit=50&page=${page}`, { signal }); sales.push(...(next.sales || [])); } return sales; }
function salesSnapshotTotals(sales) { const available = sales.length > 0 && sales.every((sale) => sale.inventoryCost !== null && sale.inventoryCost !== undefined && sale.grossProfit !== null && sale.grossProfit !== undefined); return { available, inventoryCost: available ? sales.reduce((sum, sale) => sum + Number(sale.inventoryCost), 0) : null, grossProfit: available ? sales.reduce((sum, sale) => sum + Number(sale.grossProfit), 0) : null }; }
...
export function DashboardPage() {
  ...
  return (
    <>
      ...
      />
      <ValuationMethodInfo card showLink />
      ...
      {!isLoading && dashboard && <>
        ...
        <section className="executive-metric-grid" aria-label="Resumen ejecutivo"><Card className="executive-metric executive-metric--sales"><DollarSign aria-hidden="true" /><span>Ventas del periodo</span><strong>{money(executive.revenue, activeBusiness.currency)}</strong><small>{executive.sales.length} ventas completadas · {executive.previousRevenue ? `${((executive.revenue - executive.previousRevenue) / executive.previousRevenue * 100).toFixed(1)}% vs. periodo anterior` : "Sin periodo anterior"}</small></Card><Card className="executive-metric"><CircleDollarSign aria-hidden="true" /><span>Costo de inventario</span><strong>{formatSafeMoney(executive.financial.inventoryCost, activeBusiness.currency, "Costo no disponible")}</strong><small>{executive.financial.available ? "Snapshot real de ventas completadas." : "Costo no disponible para una o más ventas."}</small></Card><Card className="executive-metric"><TrendingUp aria-hidden="true" /><span>Utilidad bruta</span><strong>{formatSafeMoney(executive.financial.grossProfit, activeBusiness.currency, "Costo no disponible")}</strong><small>{executive.financial.available ? "Ventas menos costo real de inventario." : "Requiere costos históricos de todas las ventas."}</small></Card><Card className="executive-metric"><Boxes aria-hidden="true" /><span>Inventario</span><strong>{money(dashboard.summary.inventoryValue, activeBusiness.currency)}</strong><small>{dashboard.summary.activeProducts} productos activos · {stockAlerts.length} alertas visibles</small></Card><Card className="executive-metric"><CreditCard aria-hidden="true" /><span>Cobranza pendiente</span><strong>{money(executive.pending, activeBusiness.currency)}</strong><small>{money(executive.overdueTotal, activeBusiness.currency)} vencido · {executive.overdueCustomers} clientes</small></Card><Card className="executive-metric"><Receipt aria-hidden="true" /><span>Gastos del periodo</span><strong>{money(executive.costTotal, activeBusiness.currency)}</strong><small>Mano de obra: {money(executive.labor, activeBusiness.currency)} · Logística: {money(executive.logistics, activeBusiness.currency)}</small></Card></section>
